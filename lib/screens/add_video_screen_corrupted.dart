...
class _AddVideoScreenState extends State<AddVideoScreen> {
  ...
  Future<void> _validateUrl() async {
    ...
    // Extraer información detallada según plataforma
    Map<String, String>? info;
    try {
      switch (platform) {
        case PlatformType.youtube:
          info = await YouTubeExtractionService.getYouTubeVideoInfo(url);
          break;
        case PlatformType.facebook:
          info = await FacebookExtractionService.getFacebookInfo(url);
          break;
        case PlatformType.instagram:
          info = await InstagramExtractionService.getInstagramInfo(url);
          break;
        case PlatformType.tiktok:
          info = await TikTokExtractionService.getTikTokInfo(url);
          break;
        case PlatformType.twitter:
          info = await TwitterExtractionService.getTwitterInfo(url);
          break;
        default:
          info = await _urlService.getOtherPlatformInfo(url, platform);
      }
      if (info != null) {
        final extractedInfo = info;
        setState(() {
          // Actualizar título, descripción y miniatura
          _titleController.text = extractedInfo['title'] ?? '';
          _descriptionController.text = extractedInfo['description'] ?? '';
          _thumbnailUrl = extractedInfo['thumbnailUrl'] ?? '';
          _isUrlValid = true;
        });
        debugPrint('Información extraída: $extractedInfo');
      } else {
        setState(() {
          _isUrlValid = false;
        });
        debugPrint('No se pudo extraer información de la URL');
      }
    } catch (e) {
